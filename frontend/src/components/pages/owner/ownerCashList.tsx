...
import { pointCashOutRequest } from "src/types";

function OwnerCashList() {
    const { storeId } = useParams();
    const navigate = useNavigate();

    const [ownerCashs, setOwnerCashs] = useState<pointCashOutRequest[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const url = `/api/v1/pointCashOutRequests/${storeId}?page=${page}&size=${pageSize}`;
                const response = await axios.get(url);
                // Page 객체 기준: content 배열만 추출
                setOwnerCashs(response.data.content);
                ...
                <div className="w-full mx-auto">
                    ...
                    <div className="overflow-x-auto bg-white rounded-[25px] border ml-8 mb-8">
                        <table className="min-w-full border-collapse text-[#000]">
                            ...
                            <tbody>
                                {ownerCashs && ownerCashs.length > 0 ? (
                                    ownerCashs.map((cashOutRequests, index) => (
                                        <tr
                                            key={cashOutRequests.id}
