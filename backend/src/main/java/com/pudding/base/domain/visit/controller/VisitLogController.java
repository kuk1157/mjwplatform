...
public class VisitLogController {
    ...
    @PostMapping("/visits/{storeNum}/{tableNumber}")
    ...
    @Operation(summary = "최근 방문 기록 조회", description = "고객 기준으로 방문기록 조회 2개만")
    @GetMapping("/customers/{customerId}/visits")
    public ResponseEntity<List<VisitLogDto>> getAllVisitLog(@PathVariable Integer customerId,
                                                            @RequestParam(defaultValue = "desc") String sort,
                                                            @RequestParam(required = false) Integer limit){
        List<VisitLogDto> visitLogs;
        if (limit == null) {
            visitLogs = visitLogService.getAllVisitLogSorted(customerId, sort);
        } else {
            visitLogs = visitLogService.getLimitedVisitLogSorted(customerId, sort, limit); // 최근 2개만 조회
        }
